...
class ResNetModel : public ModelUtil {
 public:
  ...
  OperatorDef *AddBlock2(int &n, const std::string &layer, int in_size,
    ...
    auto op = AddRes2(std::to_string(n++), layer, in_size, out_size, stride, train);
    for (int i = 1; i < depth; i++) {
      op = AddRes2(std::to_string(n++), op->output(0), out_size, out_size, 1, train);
    }
    return op;
  }
  ...
  OperatorDef *AddBlock3(int &n, const std::string &layer, int in_size,
                         int out_size, int stride, int depth, bool train) {
    auto op = AddRes3(std::to_string(n++), layer, in_size, out_size, stride, train);
    for (int i = 1; i < depth; i++) {
      op = AddRes3(std::to_string(n++), op->output(0), out_size, out_size, 1, train);
    }
    return op;
  }
  ...
  void Add(int type, int out_size, bool train = false) {
    ...
    if (type < 50) {
      ...
      layer = AddEnd(std::to_string(n++), layer, 512, out_size, 1)->output(0);
    } else {
      layer = AddFirst("1", layer, 64, 2, train)->output(0);
      ...
      layer = AddEnd(std::to_string(n++), layer, 2048, out_size, 1)->output(0);
    }
