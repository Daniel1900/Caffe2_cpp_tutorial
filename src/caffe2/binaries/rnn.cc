...
C10_DEFINE_string(train_data, "res/shakespeare.txt",
...
void run() {
  ...
  // self.hidden_size], dtype=np.float32))
  {
    ...
    std::vector<int64_t> dims({1, FLAGS_batch, FLAGS_hidden_size});

    Tensor value(dims, CPU);
    for(int i = 0; i < data.size(); i++)
    {
      value.mutable_data<float>()[i] = data[i];
    }
    //auto blob = workspace.CreateBlob(hidden_output);
    //BlobSetTensor(blob, caffe2::Tensor(value));
  ...
  while (num_iter < FLAGS_iters) {
    ...
    // self.batch_size, dtype=np.int32))
    {
      ...
      std::vector<int64_t> dims({FLAGS_batch});

      TensorCPU value(dims, CPU);
      for(int i = 0; i < data.size(); i++)
      {
        value.mutable_data<int>()[i] = data[i];
      }
    ...
    {
      std::vector<int64_t> dims({FLAGS_seq_length, FLAGS_batch, D});

      TensorCPU value(dims, CPU);
      for(int i = 0; i < input.size(); i++)
      {
        value.mutable_data<float>()[i] = input[i];
      }
      BlobUtil(*workspace.CreateBlob("input_blob")).Set(value, true);
    }
    // >>> workspace.FeedBlob('target', target)
    {
      std::vector<int64_t> dims({FLAGS_seq_length * FLAGS_batch});

      TensorCPU value(dims, CPU);
      for(int i = 0; i < target.size(); i++)
      {
        value.mutable_data<int>()[i] = target[i];
      }
    ...
    if (num_iter % FLAGS_iters_to_report == 0) {
      ...
      for (auto i = 0; i < FLAGS_gen_length; i++) {
        ...
        // >>> workspace.FeedBlob("input_blob", input)
        {
          std::vector<int64_t> dims({1, FLAGS_batch, D});
