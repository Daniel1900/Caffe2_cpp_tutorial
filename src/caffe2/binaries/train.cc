...
C10_DEFINE_string(folder, "", "Folder with subfolders with images");

C10_DEFINE_string(db_type, "leveldb", "The database type.");
C10_DEFINE_int(size, 224, "The image file.");
C10_DEFINE_int(iters, 1000, "The of training runs.");
C10_DEFINE_int(test_runs, 50, "The of training runs.");
C10_DEFINE_int(batch, 64, "Training batch size.");
C10_DEFINE_double(lr, 1e-4, "Learning rate.");

C10_DEFINE_bool(display, false,
                   "Show worst correct and incorrect classification.");
C10_DEFINE_bool(reshape, false, "Reshape output (necessary for squeeznet)");
C10_DEFINE_bool(matrix, false, "Show test result matrix");
