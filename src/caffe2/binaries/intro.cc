...
void print(const Blob* blob, const std::string& name) {
  ...
  std::cout << name << "(" << tensor.sizes().vec()
...
void run() {
  ...
  // >>> workspace.FeedBlob("my_x", x)
  {
    ...
    {
      value.mutable_data<float>()[i] = x[i];
    }
    auto blob = workspace.CreateBlob("my_x");
    auto tensor = blob->GetMutable<TensorCPU>();
    if (!tensor->defined())
      BlobSetTensor(blob, Tensor(CPU));

    tensor = blob->GetMutable<TensorCPU>();
    tensor->Resize(dims);
  ...
  // >>> workspace.FeedBlob("data", data)
  {
    ...
    std::vector<int64_t> dims({16, 100});
    TensorCPU value(dims, CPU);
  ...
  // >>> workspace.FeedBlob("label", label)
  {
    ...
    {
      value.mutable_data<int>()[i] = label[i];
    }
    if (!tensor->defined())
      BlobSetTensor(blob, Tensor(CPU));
