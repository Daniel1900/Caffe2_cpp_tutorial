...
C10_DEFINE_string(model, "", "Name of one of the pre-trained models.");
C10_DEFINE_string(layer, "",
                     "Name of the layer on which to split the model.");
C10_DEFINE_int(channel, -1, "The first channel to run.");
C10_DEFINE_int(batch, 1, "The number of channels to process in parallel.");
C10_DEFINE_int(size, 400, "The goal image size.");

C10_DEFINE_string(file, "", "The image file.");
C10_DEFINE_int(iters, 60, "The of training runs.");
C10_DEFINE_int(scale_runs, 10, "The amount of iterations per scale.");
C10_DEFINE_int(percent_incr, 40, "Percent increase per round.");
C10_DEFINE_int(initial, -17, "The of initial value.");
C10_DEFINE_double(lr, 1, "Learning rate.");
C10_DEFINE_bool(display, false, "Show image while dreaming.");
