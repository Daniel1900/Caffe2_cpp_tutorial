...
TensorCPU TensorUtil::ScaleImageTensor(int width, int height) {
  ...
  std::vector<int64_t> dims({count, dim_c, height, width});
  TensorCPU ts_cpu(dims, CPU);
  ...
  {
    ts_cpu.mutable_data<float>()[i] = output[i];
  }
  return ts_cpu;
}
...
void image_to_tensor(TensorCPU &tensor, cv::Mat &image, float mean = 128) {
  ...
  std::vector<int64_t> dims({1, 3, image.rows, image.cols});
...
void read_image_tensor(TensorCPU &tensor,
  ...
  // create tensor
  std::vector<int64_t> dims({(int64_t)indices.size(), 3, height, width});
  TensorCPU t(dims, CPU);

  for (int i = 0; i < data.size(); i++)
  {
    t.mutable_data<float>()[i] = data[i];
  }
...
void tensor_print_type(const Tensor &tensor, const std::string &name,
  ...
  if (name.length() > 0) std::cout << name << "(" << tensor.sizes().vec() << "): ";
