...
void Series::dot(void *b, int x, int y, int r) const {
  Trans trans(b);
  cv::circle(trans.with(color_), {x, y}, r, color2scalar(color_), -1, 16);
}
  ...
  switch (type_) {
    ...
    case RangeLine: {
      ...
      for (const auto &e : entries_) {
        ...
        if (type_ == DotLine || type_ == Dots) {
          cv::circle(trans.with(color_), point, 2, color, 1, 16);
        }
        last_x = x, last_y = y;
      }
    } break;
      ...
      for (const auto &e : entries_) {
        ...
        if (type_ == Horizontal) {
          cv::line(
              ...
              {(int)(x_max * xs + xd), (int)(y * ys + yd)}, color, 1, 16);
        } else if (type_ == Vertical) {
          cv::line(
              trans.with(color_), {(int)(y * xs + xd), (int)(y_min * ys + yd)},
              {(int)(y * xs + xd), (int)(y_max * ys + yd)}, color, 1, 16);
        }
      }
    } break;
    case Range: {
      ...
      for (const auto &e : entries_) {
        ...
        if (has_last) {
          ...
          cv::fillConvexPoly(trans.with(color_), points, 4, color, 16);
        } else {
          has_last = true;
        }
    ...
    case Circle: {
      for (const auto &e : entries_) {
        ...
        cv::Point point((int)(x * xs + xd), (int)(y * ys + yd));
        cv::circle(trans.with(color_), point, r, color, -1, 16);
      }
    } break;
  }
}
...
void Figure::draw(void *b, float x_min, float x_max, float y_min, float y_max,
  ...
  cv::rectangle(trans.with(sub_axis_color_), {border_size_, border_size_},
                ...
                color2scalar(sub_axis_color_), 1, 16);
  ...
  for (const auto &s : series_) {
    ...
    cv::circle(trans.with(background_color_),
               ...
               color2scalar(background_color_), -1, 16);
